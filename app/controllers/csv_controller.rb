class CsvController < ApplicationController
  ...
  def import
    @file = Roo::Spreadsheet.open('/csv/numbers.xlsx')
    @file.each do |row|
      @phone_number = row[1]
      @category = row[2]
      @region = row[3]
      ...
      @name = row[9]
      @number = Number.new
      @number.update(phone: @phone_number, category: @category, region: @region, country: @country, site: @site, name: @name)
    end
  end

  def ghidtur_import
    @file = Roo::Spreadsheet.open('/csv/ghidtur.xlsx')
    @file.each do |row|
